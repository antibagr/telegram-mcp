...
class _FakeMcp:
    """mcp 2.x takes host/port/transport_security per run call, not on a settings object."""
    ...
    async def run_sse_async(self, **kwargs):
        self.ran, self.kwargs = "sse", kwargs

    async def run_streamable_http_async(self, **kwargs):
        self.ran, self.kwargs = "http", kwargs
...
async def test_serve_http_leaves_transport_security_unset_by_default(monkeypatch):
    ...
    assert fake.kwargs["transport_security"] is None
