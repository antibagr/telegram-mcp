...
@pytest.mark.asyncio
async def test_send_rich_carries_silent_flag():
    cl = _FakeClient(premium=True)
    await messages._send_rich(cl, "peer", "| a |", "rich", silent=True)
...
@pytest.mark.asyncio
async def test_send_rich_defaults_to_not_silent():
    cl = _FakeClient(premium=True)
    await messages._send_rich(cl, "peer", "| a |", "rich")
